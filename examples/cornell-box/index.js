import { Box, Mesh, PerspectiveCamera, Scene, SolidColor, BlinnPhong, AmbientLight, PointLight, Plane, Metal, Rasterizer, } from '../../src/index.js';
...
renderer.init().then(() => {
    ...
    const backWall = new Mesh(plane, new BlinnPhong([180 / 255, 180 / 255, 180 / 255]));
    vec3.set(0, 0, -0.5, backWall.localPosition);
    // quat.fromEuler(0, 0, 0, 'xyz', backWall.localQuaternion);
    const ground = new Mesh(plane, new BlinnPhong([180 / 255, 180 / 255, 180 / 255]));
    vec3.set(0, -0.5, 0, ground.localPosition);
    quat.fromEuler(-Math.PI / 2, 0, 0, 'xyz', ground.localQuaternion);
    const ceiling = new Mesh(plane, new BlinnPhong([180 / 255, 180 / 255, 180 / 255]));
    ...
    const shortBox = new Mesh(box, new BlinnPhong([180 / 255, 180 / 255, 180 / 255]));
    ...
    scene.add(leftWall, rightWall, backWall, ground, ceiling, tallBox, shortBox, ambient, light, lightMesh);
