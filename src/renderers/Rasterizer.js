...
import { BLINN_PHONG } from '../materials/constants.js';
class Rasterizer {
    constructor(canvas) {
    ...
    render(scene, camera) {
        ...
        const { vertexData, indexData, materialData, lightData } = this.getSceneData(scene);
        ...
        const transformedCameraPosition = vec3.transformMat4([0, 0, 0], mat4.translation(camera.localPosition));
        const viewMatrix = mat4.lookAt(transformedCameraPosition, vec3.create(0, 0, 0), UP);
        const viewProjectionMatrix = mat4.multiply(camera.projectionMatrix, viewMatrix);
        this.device.queue.writeBuffer(this.viewProjectionMatrixBuffer, 0, viewProjectionMatrix);
        this.device.queue.writeBuffer(this.cameraPositionBuffer, 0, transformedCameraPosition);
    ...
    setRenderPassDescriptor() {
        ...
        if (!this.depthTexture) {
            throw new Error('Depth texture has not been set. Call .setDepthTexture() first.');
        }
        const canvasTexture = this.context.getCurrentTexture();
        this.renderPassDescriptor = {
