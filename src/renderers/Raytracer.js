import { Mesh } from '../primitives/exports.js';
import { mat4, quat, vec3 } from 'wgpu-matrix';
import { UP } from '../constants.js';
import { BLINN_PHONG, METAL, } from '../materials/exports.js';
...
class Raytracer {
    ...
    async init() {
        ...
        if (!device)
            throw Error('Could not request WebGPU logical device.');
        this.device = device;
    ...
    render(scene, camera, frame = 0) {
        ...
        this.device.queue.writeBuffer(this.frameNumberBuffer, 0, new Uint32Array([frame]));
        this.device.queue.writeBuffer(this.cameraPositionBuffer, 0, camera.localPosition // TODO: consider global position
        );
        // Calculate viewport data
        const lookAt = [0, 0, 0];
        const focalLength = vec3.length(vec3.subtract(camera.localPosition, lookAt));
        ...
        const viewportUpperLeft = vec3.sub(vec3.sub(vec3.sub(camera.localPosition, vec3.mulScalar(w, focalLength)), halfViewportU), halfViewportV);
        ...
        if (!this.vertexData ||
            ...
            new Float32Array(this.materialBuffer.getMappedRange()).set(this.materialData);
    ...
    updateCanvas() {
        ...
        this.device.queue.writeBuffer(this.resolutionBuffer, 0, new Uint32Array([this.canvas.width, this.canvas.height]));
    }
    getSceneData(scene) {
        ...
        scene.traverse((group, globalPosition, globalRotation, globalScale) => {
            ...
            if (group instanceof Mesh) {
                ...
                mesh.geometry.forEachVertex((_index, position, normal, uv) => {
                    ...
                    const transformedNormal = vec3.transformMat4(normal, worldMatrixInverseTranspose);
            ...
            if (group instanceof Light) {
                const light = group;
                const transformedLightPosition = vec3.transformMat4([0, 0, 0], worldMatrix);
    ...
    setRenderPassDescriptor() {
        ...
        if (!this.context) {
            throw new Error('Canvas context has not been set. Call .setCanvasFormatAndContext() first.');
        }
        const canvasTexture = this.context.getCurrentTexture();
        this.renderPassDescriptor = {
    ...
    setVertexBuffer() {
        ...
        new Float32Array(this.frameBufferViewVertexBuffer.getMappedRange()).set(vertexData);
    ...
    setComputePipeline() {
        ...
        if (!this.bindGroupLayout) {
            throw new Error('Bind group layout has not been set. Call .setBindGroup() first.');
        }
    ...
    setRenderPipeline() {
        ...
        if (!this.format) {
            throw new Error('Canvas format has not been set. Call .setCanvasFormatAndContext() first.');
        }
        if (!this.bindGroupLayout) {
            throw new Error('Bind group layout has not been set. Call .setBindGroup() first.');
        }
        if (!this.pipelineLayout) {
            throw new Error('Pipeline layout has not been set. Call .setComputePipeline() first.');
        }
